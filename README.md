...
- `runAutoBattle()` 目前仍保留作為時間軸數值驗證入口；世界戰鬥、時間軸戰鬥與 replay 的 queue、controller、preview、outcome、reward、cleanup 與 lifecycle，現在都已集中到 `battleSystem.ts` 的同一套 battle core。
- `Adventure` 目前已退回 UI bridge：頁面只負責 state apply、visual dispatch 與 Redux 串接；其中 battle 相關的 React/Redux 套用也已開始收斂到 `createAdventureBattleUiBridge(...)`，visual dispatch 也已開始收斂到 `createAdventureBattleVisualBridge(...)`，不再自己持有 live world / replay 的 battle 規則與主控判定。
