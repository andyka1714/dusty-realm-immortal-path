...
- `Adventure` 目前已退回 UI bridge：頁面只負責 state apply、visual dispatch 與 Redux 串接；其中 battle 相關的 React/Redux 套用也已開始收斂到 `createAdventureBattleUiBridge(...)`，不再自己持有 live world / replay 的 battle 規則與主控判定。
