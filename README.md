...
- `runAutoBattle()` 目前仍保留作為時間軸數值驗證入口；世界戰鬥、時間軸戰鬥與 replay 的 queue、controller、preview、outcome、reward、cleanup 與 lifecycle，現在都已集中到同一套 battle core，而 `battleSystem.ts` 已退成 barrel / shared type 入口。實際 orchestration 目前分散在 `battleAutoTimeline.ts`、`battleReplay.ts`、`battleLifecycle.ts`、`battleWorldStrike.ts`、`battleWorldController.ts`、`battleEncounter.ts`、`battleTurnPhases.ts`、`battleAftermath.ts`、`battleStatuses.ts`、`battleStatusEffects.ts`、`battleStats.ts` 等專責模組；其中 `battleReplay / battleLifecycle / battleAutoTimeline / battleStatuses / battleStatusEffects` 也已再往下拆成更細的 state、runner、timers、visual、builders、applications 與 ticks 子模組。
