...
- `runAutoBattle()` 目前仍保留作為時間軸數值驗證入口；世界戰鬥、時間軸戰鬥與 replay 的 queue、controller、preview、outcome、reward、cleanup 與 lifecycle，現在都已集中到同一套 battle core，其中 `battleSystem.ts` 持有主流程，`battleLog.ts`、`battlePassives.ts`、`battleStatuses.ts`、`battleStatusEffects.ts`、`battleProfiles.ts`、`battleTiming.ts`、`battleEncounter.ts`、`battleTimeline.ts`、`battleRewards.ts`、`battleReplay.ts`、`battleLifecycle.ts`、`battleWorldStrike.ts`、`battleWorldController.ts`、`battleTurnPhases.ts` 與 `battleTimelineResults.ts` 已開始分別承接 combat log / snapshot provider 容器、passive flags / canonical skill / passive 判定、status snapshot / DOT tick / status label、技能/特招狀態生成與套用、skill / special timeline profile 與攻速 / 冷卻解析、opening / upkeep / runtime context / encounter seed、timing、auto-battle loop state / runner、reward、replay controller / state / visual / finish、lifecycle、world-strike、world combat controller、player / enemy turn phase，以及 auto-battle 的結果收尾與戰利品結算。
