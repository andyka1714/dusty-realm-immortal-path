...
- battle 第二輪整理也已開始把純函式自 `battleSystem.ts` 分流：`battleRewards.ts` 現在承接 reward manifest / application plan，`battleReplay.ts` 承接 replay state、visual 與 finish plan；後續 battle 擴充會優先沿這個模組邊界拆分，而不是回頭把純資料組裝塞回頁面
