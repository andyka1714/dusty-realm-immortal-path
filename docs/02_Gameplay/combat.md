...
- `Adventure` 目前已退回 battle UI bridge：live world / replay 的 battle 規則、controller、preview、outcome、reward、cleanup 與 lifecycle 均已集中到 `battleSystem.ts`，而頁面上的 React/Redux 套用與 visual dispatch 也已開始分別收斂到 `createAdventureBattleUiBridge(...) / createAdventureBattleVisualBridge(...)`；頁面本體只剩 state apply、visual dispatch 與 Redux/UI bridge
