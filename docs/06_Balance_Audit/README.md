...
- battle 第二輪整理目前已開始把純函式自 `battleSystem.ts` 分流到專責模組：`battleRewards.ts` 承接 reward manifest / application plan，`battleReplay.ts` 承接 replay state、visual 與 finish plan，`battleLifecycle.ts` 承接 timer / replay transition / respawn / encounter reset；後續若再整理 battle core，應沿這個模組邊界拆分而不是回頭擴大頁面橋接層
