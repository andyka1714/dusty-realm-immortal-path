...
### Requirement: 裝備必須按職業與境界形成 progression
系統必須 (MUST) 讓裝備物品線按職業與境界形成可驗證 progression。

#### Scenario: 玩家進入新境界
- **WHEN** 玩家抵達新的主要境界
- **THEN** 該境界必須能找到對應劍修、體修、法修或通用裝備承接
- **AND** 裝備必須能被煉器、任務獎勵、掉落或商店來源引用
...
### Requirement: 煉器材料必須支撐裝備與法寶 progression
系統必須 (MUST) 提供按境界與職業分層的煉器材料，支撐裝備打造、任務提交與後續法寶線。

#### Scenario: 玩家打造職業裝備
- **WHEN** 玩家透過煉器 recipe 打造職業裝備
- **THEN** recipe 必須消耗對應境界與職業語彙合理的煉器材料
- **AND** 這些材料必須能從怪物、採集、商店、任務或 Boss 取得
