...
- **AND** 必須包含 `soul` 區塊，承接 merit、lifetime stats、perk 解鎖、heirloom vault 與輪迴暫存資訊

#### Scenario: 輪迴僅重置 current run
- **WHEN** 玩家完成輪迴並開始新一世
- **THEN** 存檔必須重建新的 `current` 區塊
- **AND** 同一份存檔內的 `soul` 區塊必須保留並寫入最新 merit / perk / heirloom 狀態
...
- **THEN** 仍依 `current.character.lastSaveTime` 計算離線收益
...
- **AND** 自動補上預設 `soul` state 與 `schemaVersion`
